...
static esp_err_t custom_ep_handler(uint32_t session_id, const uint8_t *inbuf, ssize_t inlen, uint8_t **outbuf, ssize_t *outlen, void *priv_data) {
  ...
  Provisioning->setUserToken((const char *)inbuf);

  // Prepare response
  const char *resp = "User token received";
...
void ProvisioningClass::beginProvision(prov_scheme_t prov_scheme, scheme_handler_t scheme_handler, wifi_prov_security_t security, const char *pop, const char *service_name, const char *service_key, uint8_t *uuid, bool reset_provisioned) {
  ...
  if (provisioned == false) {
...
#endif
    // Create and register custom endpoint for user_token
    if (wifi_prov_mgr_endpoint_create("user-token") != ESP_OK) {
      log_e("Failed to create custom endpoint");
      wifi_prov_mgr_deinit();
      return;
    }
    ...
    if (wifi_prov_mgr_endpoint_register("user-token", custom_ep_handler, (void *)this) != ESP_OK) {
      log_e("Failed to register custom endpoint handler");
      wifi_prov_mgr_deinit();
      return;
    }
  } else {
...
const char *ProvisioningClass::getUserToken() const {
  return user_token;
}
